...
-- 2)List all shows hosted by "Vrajesh Hirjee"?
     ...
     SELECT h.HostName , s.ShowName
     FROM Hosts h JOIN Shows s
     USING (HostID) 
     WHERE h.HostName = "Vrajesh Hirjee";
     
-- 3)Count the number of awards each show has won?
     
     SELECT s.ShowName, COUNT(a.AwardID) AS NumberOfAwards
     FROM Shows s JOIN Awards a
     USING (ShowID)
     GROUP BY s.ShowName;
     
-- 4)Find shows that have partnerships with "Spotify"?

     SELECT DISTINCT s.ShowID, s.ShowName, p.PartnerName
     FROM Shows s
...
-- 7)Find the total count of shows for each host?
	 ...
	 SELECT HostName, SUM(ShowCount) AS NumberOfShows FROM Hosts
     GROUP BY HostName;
...
-- 9)Retrieve the stations in the "South" region launched after 2010?
     ...
     SELECT StationName, Location, LaunchDate 
     FROM Stations
...
-- Find Shows with Awards and Their Partnership Information
   SELECT
    S.ShowID,
    S.ShowName,
    A.AwardName,
    ...
    P.PartnerName
    FROM Shows S
    JOIN Awards A ON S.ShowID = A.ShowID
...
-- Find the Year with the Most Awards

SELECT
    A.Year,
    COUNT(A.AwardID) AS AwardCount
FROM Awards A
GROUP BY A.Year
HAVING COUNT(A.AwardID) = (SELECT COUNT(AwardID) FROM Awards GROUP BY Year ORDER BY COUNT(AwardID) DESC LIMIT 1)
ORDER BY A.Year;
